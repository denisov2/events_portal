...
function eventchamp_events_search_results_output( $atts, $content = null ) {
  ...
  //  var_dump($startdate);
    ...
    if (!empty($wp_query) || !empty($args_no_ads)) {
        ...
        while ($wp_query_no_ads->have_posts()) {
            $wp_query_no_ads->the_post();
